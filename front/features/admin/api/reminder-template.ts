import {
  ...
  type BackendTemplateContextPage,
} from "@/shared/api";
...
export const reminderTemplateApi = {
  ...
  contexts: (path: string, cursor?: string) => request<BackendTemplateContextPage>(
    cursor ? `${path}${path.includes("?") ? "&" : "?"}${new URLSearchParams({ cursor })}` : path),
