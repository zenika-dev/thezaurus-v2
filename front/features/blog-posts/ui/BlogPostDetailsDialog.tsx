...
export function BlogPostDetailsDialog({ post, open, onClose, onUpdate, onDelete }: BlogPostDetailsDialogProps) {
  ...
  useEffect(() => {
    if (post && open) {
      ...
      setCreationDate(post.creationDate ? dayjs(post.creationDate, "DD-MM-YYYY") : null);
      setExpectedPublicationDate(post.expectedPublicationDate ? dayjs(post.expectedPublicationDate, "DD-MM-YYYY") : null);
      setTags(post.tags ?? []);
  ...
  const handleSave = () => {
    ...
    if (expectedPublicationDate?.isBefore(creationDate)) {
      setToastMsg("La date de publication doit être supérieure à la date de création.");
      setToastOpen(true);
      return;
    }
  ...
  const handleDelete = () => {
    if (post && window.confirm("Êtes-vous sûr de vouloir supprimer ce post ?")) {
      onDelete(post.id);
      onClose();
    }
  };
  ...
  const { text: statusColor, bg: statusBg } = blogPostStatusConfig[status];
    ...
    <Dialog onClose={onClose} open={open} maxWidth="md" fullWidth>
      <DialogTitle className="pb-0! flex! justify-between! items-center!">
        ...
        <FormControl size="small" className="min-w-30">
          <Select value={status} onChange={(e) => setStatus(e.target.value as BlogPostStatus)}
            style={{ fontWeight: "bold", color: statusColor }}
            slotProps={{ input: { style: { borderColor: statusBg } } }}>
      ...
      <DialogContent>
        <p className="text-sm text-text-muted mb-6">Modifier les détails de l&apos;article</p>

        <div className="flex flex-col gap-4">
          <TextField label="Titre du post" required fullWidth
            value={title} onChange={(e) => setTitle(e.target.value)} />

          <div className="grid grid-cols-2 gap-4">
            <TextField label="Auteur" required fullWidth
              value={author} onChange={(e) => setAuthor(e.target.value)} />

            <FormControl fullWidth required>
              <InputLabel id="edit-post-tags-label">Tags</InputLabel>
              <Select labelId="edit-post-tags-label" multiple value={tags} label="Tags"
                onChange={(e) => {
                  const v = e.target.value;
                  setTags(typeof v === "string" ? v.split(",") : (v as string[]));
                }}
                renderValue={(sel) => sel.map((v) => blogPostTags[v] ?? v).join(", ")}>
                {Object.entries(blogPostTags).map(([v, l]) => <MenuItem key={v} value={v}>{l}</MenuItem>)}
              </Select>
            </FormControl>

            <DatePicker label="Date" value={creationDate} onChange={setCreationDate}
              views={["year", "month", "day"]} format="DD/MM/YYYY"
              slotProps={{ textField: { fullWidth: true, required: true } }} />

            <DatePicker label="Date de publication souhaitée" value={expectedPublicationDate}
              onChange={setExpectedPublicationDate}
              views={["year", "month", "day"]} format="DD/MM/YYYY"
              slotProps={{ textField: { fullWidth: true } }} />

            <TextField label="Lien blog Zenika" placeholder="https://..."
              value={zenikaBlogLink} onChange={(e) => setZenikaBlogLink(e.target.value)} fullWidth
              slotProps={{ input: {
                startAdornment: <InputAdornment position="start"><Library size={16} /></InputAdornment>,
                endAdornment: zenikaBlogLink && isValidUrl(zenikaBlogLink) ? (
                  <InputAdornment position="end">
                    <IconButton size="small" component="a" href={zenikaBlogLink} target="_blank" rel="noopener noreferrer" className="text-primary! p-0.5!">
            ...
            <TextField label="Lien draft Google Doc" placeholder="https://..."
              value={googleDocDraftLink} onChange={(e) => setGoogleDocDraftLink(e.target.value)} fullWidth
              slotProps={{ input: {
                startAdornment: <InputAdornment position="start"><FileText size={16} /></InputAdornment>,
                endAdornment: googleDocDraftLink && isValidUrl(googleDocDraftLink) ? (
                  <InputAdornment position="end">
                    <IconButton size="small" component="a" href={googleDocDraftLink} target="_blank" rel="noopener noreferrer" className="text-primary! p-0.5!">
                      <ExternalLinkIcon size={14} />
                    </IconButton>
                  </InputAdornment>
                ) : null,
      ...
      <Snackbar open={toastOpen} autoHideDuration={4000} onClose={() => setToastOpen(false)}
        ...
        <Alert onClose={() => setToastOpen(false)} severity="error" variant="filled">
          {toastMsg}
        </Alert>
      </Snackbar>
    </Dialog>
